matrix.p.sig<-function(comm, dist.spp, envir, analysis = c("adonis", "mantel"), method = "bray", squareroot = TRUE, method.envir = "euclidean", runs = 999){
	...
	p.dist <- vegdist(p.matrix, method = method)
	if (squareroot == TRUE) {
		p.dist<-sqrt(p.dist)
	}
	if(analysis == 1){
		mod_obs<-adonis(p.dist~envir,permutations=runs)
		statistic_obs<-mod_obs$aov.tab$F.Model[1]
		p_obs<-mod_obs$aov.tab$"Pr(>F)"[1]
	}
	if(analysis == 2){
		env.dist<-vegdist(envir, method = method.envir)
		mod_obs<-mantel(p.dist,env.dist,permutations=runs)
	...
	for(k in 1:runs){
		...
		p.dist_null<-vegdist(m_p_null,method=method)
		if(squareroot == TRUE){
			p.dist_null<-sqrt(p.dist_null)
		}
		if(analysis == 1){
			mod_null<-adonis(p.dist_null~envir,permutations=1)
			res_null[k,]<-mod_null$aov.tab$F.Model[1]
		}
		if(analysis == 2){
			mod_null<-mantel(p.dist_null,env.dist,permutations=1)
